...
// ======================================================================
//
// doctest.h - the lightest feature-rich C++ single-header testing framework for unit ns_testing_context and TDD
//
...
struct ContextOptions //! OCLINT too many fields
{
    ...
    String order_by;    // how ns_testing_context should be ordered
    unsigned rand_seed; // the seed for rand ordering

    unsigned first; // the first (matching) test to be executed
    unsigned last;  // the last (matching) test to be executed

    int abort_after;           // stop ns_testing_context after this many failed assertions
    int subcase_filter_levels; // apply the subcase filters for the first N levels

    bool success;              // include successful assertions in output
    bool case_sensitive;       // if filtering should be case sensitive
    bool exit;                 // if the program should be exited after the ns_testing_context are ran/whatever
    bool duration;             // print the time duration of each test case
    bool minimal;              // minimal console output (only test failures)
    bool quiet;                // no console output
    bool no_throw;             // to skip exceptions-related assertion macros
    bool no_exitcode;          // if the framework should return 0 as the exitcode
    bool no_run;               // to not run the ns_testing_context at all (can be done with an "*" exclude)
    ...
    bool count;            // if only the count of matching ns_testing_context is to be retrieved
    bool list_test_cases;  // to list all ns_testing_context matching the filters
...
#ifdef DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS
    template<typename T>
    explicit Approx(const T &value, typename detail::enable_if<std::is_constructible<double, T>::value>::type * = static_cast<T *>(nullptr)) {
...
// for registering ns_testing_context with a fixture
        ...
        struct iter<std::tuple<Type, Rest...>> { \
            iter(const char *file, unsigned line, int index) { \
                doctest::detail::regTest( \
                        doctest::detail::TestCase(func<Type>, file, line, doctest_detail_test_suite_ns::getCurrentTestSuite(), doctest::detail::type_to_string<Type>(), int(line) * 1000 + index) * \
                        dec); \
...
// for grouping ns_testing_context in test suites by using code blocks
...
#    define DOCTEST_TEST_SUITE_BEGIN(decorators) \
        DOCTEST_GLOBAL_NO_WARNINGS(DOCTEST_ANONYMOUS(DOCTEST_ANON_VAR_), doctest::detail::setTestSuite(doctest::detail::TestSuite() * decorators)) \
        static_assert(true, "")

// for ending a testsuite block
#    define DOCTEST_TEST_SUITE_END \
        DOCTEST_GLOBAL_NO_WARNINGS(DOCTEST_ANONYMOUS(DOCTEST_ANON_VAR_), doctest::detail::setTestSuite(doctest::detail::TestSuite() * "")) \
        typedef int DOCTEST_ANONYMOUS(DOCTEST_ANON_FOR_SEMICOLON_)

// for registering exception translators
#    define DOCTEST_REGISTER_EXCEPTION_TRANSLATOR_IMPL(translatorName, signature) \
        inline doctest::String translatorName(signature); \
        DOCTEST_GLOBAL_NO_WARNINGS(DOCTEST_ANONYMOUS(DOCTEST_ANON_TRANSLATOR_), doctest::registerExceptionTranslator(translatorName)) \
...
#    define DOCTEST_REGISTER_REPORTER(name, priority, reporter) \
        DOCTEST_GLOBAL_NO_WARNINGS(DOCTEST_ANONYMOUS(DOCTEST_ANON_REPORTER_), doctest::registerReporter<reporter>(name, priority, true)) \
        static_assert(true, "")

// for registering listeners
#    define DOCTEST_REGISTER_LISTENER(name, priority, reporter) \
        DOCTEST_GLOBAL_NO_WARNINGS(DOCTEST_ANONYMOUS(DOCTEST_ANON_REPORTER_), doctest::registerReporter<reporter>(name, priority, false)) \
        static_assert(true, "")
...
#        define DOCTEST_ASSERT_IMPLEMENT_1(assert_type, ...) \
            DOCTEST_CLANG_SUPPRESS_WARNING_WITH_PUSH("-Woverloaded-shift-op-parentheses") \
            doctest::detail::decomp_assert(doctest::assertType::assert_type, __FILE__, __LINE__, #__VA_ARGS__, doctest::detail::ExpressionDecomposer(doctest::assertType::assert_type) << __VA_ARGS__) \
                    DOCTEST_CLANG_SUPPRESS_WARNING_POP
...
#        define DOCTEST_BINARY_ASSERT(assert_type, comparison, ...) \
            doctest::detail::binary_assert<doctest::detail::binaryAssertComparison::comparison>(doctest::assertType::assert_type, __FILE__, __LINE__, #__VA_ARGS__, __VA_ARGS__)
...
// for registering ns_testing_context in classes
...
// this holds both parameters from the command line and runtime data for ns_testing_context
...
// all the registered ns_testing_context
...
namespace {
using namespace detail;
// for sorting ns_testing_context by file/line
...
// for sorting ns_testing_context by suite/file/line
...
// for sorting ns_testing_context by name/suite/file/line
...
namespace detail {
// used by the macros for registering ns_testing_context
...
ResultBuilder::ResultBuilder(assertType::Enum at, const char *file, int line, const char *expr, const char *exception_type, const char *exception_string) {
...
void fulltext_log_assert_to_stream(std::ostream &s, const AssertData &rb) {
    ...
    if(rb.m_at & assertType::is_throws) { //! OCLINT bitwise operator in conditional
        ...
        s << Color::Cyan << assertString(rb.m_at) << "( " << rb.m_expr << ", \"" << rb.m_exception_string << "\", " << rb.m_exception_type << " ) " << Color::None;
        if(rb.m_threw) {
            if(!rb.m_failed) {
                s << "threw as expected!\n";
            } else {
        ...
        s << Color::Cyan << assertString(rb.m_at) << "( " << rb.m_expr << ", \"" << rb.m_exception_string << "\" ) " << Color::None
...
#        endif // DOCTEST_PLATFORM_WINDOWS
        ...
        xml.startElement("testsuite")
                ...
                .writeAttribute("ns_testing_context", p.numAsserts);
...
struct ConsoleReporter : public IReporter {
    ...
    void printHelp() {
        ...
        // clang-format off
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "c,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "count                         "
              << Whitespace(sizePrefixDisplay*1) << "prints the number of matching ns_testing_context\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ltc, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "list-test-cases               "
              << Whitespace(sizePrefixDisplay*1) << "lists all matching ns_testing_context by name\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tc,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "test-case=<filters>           "
              << Whitespace(sizePrefixDisplay*1) << "filters     ns_testing_context by their name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tce, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "test-case-exclude=<filters>   "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT ns_testing_context by their name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "sf,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "source-file=<filters>         "
              << Whitespace(sizePrefixDisplay*1) << "filters     ns_testing_context by their file\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "sfe, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "source-file-exclude=<filters> "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT ns_testing_context by their file\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ts,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "test-suite=<filters>          "
              << Whitespace(sizePrefixDisplay*1) << "filters     ns_testing_context by their test suite\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tse, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "test-suite-exclude=<filters>  "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT ns_testing_context by their test suite\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ob,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "order-by=<string>             "
              << Whitespace(sizePrefixDisplay*1) << "how the ns_testing_context should be ordered\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "e,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "exit=<bool>                   "
              << Whitespace(sizePrefixDisplay*1) << "exits after the ns_testing_context finish\n";
    ...
    void test_run_end(const TestRunStats &p) override {
        ...
        s << Color::Cyan << "[doctest] " << Color::None << "test cases: " << std::setw(totwidth) << p.numTestCasesPassingFilters << " | "
          << ((p.numTestCasesPassingFilters == 0 || anythingFailed) ? Color::None : Color::Green) << std::setw(passwidth) << p.numTestCasesPassingFilters - p.numTestCasesFailed << " passed"
          << Color::None << " | " << (p.numTestCasesFailed > 0 ? Color::Red : Color::None) << std::setw(failwidth) << p.numTestCasesFailed << " failed" << Color::None << " |";
        if(opt.no_skipped_summary == false) {
            const int numSkipped = p.numTestCases - p.numTestCasesPassingFilters;
            s << " " << (numSkipped == 0 ? Color::None : Color::Yellow) << numSkipped << " skipped" << Color::None;
        }
        s << "\n";
        s << Color::Cyan << "[doctest] " << Color::None << "assertions: " << std::setw(totwidth) << p.numAsserts << " | " << ((p.numAsserts == 0 || anythingFailed) ? Color::None : Color::Green)
          << std::setw(passwidth) << (p.numAsserts - p.numAssertsFailed) << " passed" << Color::None << " | " << (p.numAssertsFailed > 0 ? Color::Red : Color::None) << std::setw(failwidth)
          << p.numAssertsFailed << " failed" << Color::None << " |\n";
        s << Color::Cyan << "[doctest] " << Color::None << "Status: " << (p.numTestCasesFailed > 0 ? Color::Red : Color::Green) << ((p.numTestCasesFailed > 0) ? "FAILURE!" : "SUCCESS!") << Color::None
          << std::endl;
    }
...
#        define DOCTEST_PARSE_STR_OPTION(name, sname, var, default) \
            if(parseOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX name "=", &strRes, default) || parseOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX sname "=", &strRes, default) || \
               withDefaults) \
            p->var = strRes
...
#        endif // DOCTEST_PLATFORM_WINDOWS
    ...
    if(!testArray.empty()) {
        if(p->order_by.compare("file", true) == 0) {
            ...
            // means no sorting - beneficial for death ns_testing_context which call into the executable
    ...
    for(auto &curr : testArray) {
        ...
        // do not execute the test if we are to only count the number of filter passing ns_testing_context
        if(p->count)
            continue;
...
#        endif // DOCTEST_CONFIG_NO_EXCEPTIONS
            ...
            // stop executing ns_testing_context if enough assertions have failed
